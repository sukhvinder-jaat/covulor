    ...
    <UModal v-model="isOpen" :overlay="true" class="relative">
      ...
      <div class="p-[37px_47px_41px_28px]">
        <form @submit.prevent="handleSubmit">
          <p
            class="font-roboto font-bold text-xl text-purple mb-[23px] pr-[18px]"
          >
            UPLOAD SCAN
          </p>
          <hr class="bg-[F2F0F9]" />
          ...
          <div class="flex justify-end mt-[28px]">
            <UButton
              type="submit"
              class="w-[142px] text-center rounded-none py-3 justify-center"
              :class="selectedRepository === '' ? 'cursor-not-allowed' : 'cursor-pointer'"
              >Upload</UButton
            >
          </div>
        </form>
