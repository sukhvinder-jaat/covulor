<template>
  <div class="my-[35px]">
    <CommonTable :header="heading">
      <tr v-for="(content, index) in repo.findings.findings" :key="index"
        ...
        <td class="px-5 py-4 whitespace-nowrap">
          <!-- popup -->
          <UPopover class="table_popup" :class="content.finding_types.length < 1 ? 'hidden' : 'block'">
            <div label="Open"
                ...
                <div class="bg-lawnGreen p-[14px] ps-7 min-w-[154px] rounded-md">
                  <ul class="list-disc flex flex-col gap-y-2 items-start justify-start">
                    <li class="font-roboto text-white font-medium text-sm"
                      v-for="(tagOption, i) in content.finding_types" :key="i">
                      {{ tagOption }}
                    </li>
                  </ul>
                </div>
        ...
        <td class="px-3 py-4 whitespace-nowrap">
          <p class="flex items-center gap-x-[3px] text-sm font-roboto font-medium text-lightGreen" :class="content.confidence < 50
            ? '!text-turquoiseBlue'
            : content.confidence < 80
              ? '!text-lawnGreen'
              : content.confidence > 80 && 'text-lightGreen'
            ">
            <UProgress :value="content.confidence" :color="content.confidence < 50
              ? 'turquoiseBlue'
              : content.confidence < 80
                ? 'lawnGreen'
