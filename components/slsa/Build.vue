  ...
  <div class="flex flex-wrap w-full mb-[25px]">
    ...
    <div class="xl:w-[80%] w-full xl:pe-[18px]">
      ...
      <div
        ...
        <div class="xl:w-3/12 md:w-4/12 xl:px-0 px-3 small:w-6/12 w-full h-full min-h-[205px]"
          v-for="(content, index) in buildThreats.findings" :class="index === 0
            ? '!xl:pl-0'
            : '' || index === buildThreats.findings.length - 1
              ? '!xl:pr-0 !xl:pe-0'
              : ''
            " :key="index">
          <Card :type="buildThreats.type" :title="content.title" :rank="content.count" :growth="content.growth" />
        </div>
      </div>
    </div>
    ...
    <div class="xl:w-[20%] flex flex-col items-center w-full xl:ps-[18px]">
      ...
      <div class="border-b border-lightGreen w-full pb-2 xl:ps-0 ps-[13px] text-center">
        <Heading title="Dependencies Threats" :isSubHeading="true" />
      </div>
      <!-- Looping through dependenciesThreats and rendering Card components -->
      <div class="xl:w-full md:w-4/12 xl:px-0 px-3 small:w-6/12 w-full mt-[38px]">
        <div class="w-full h-full min-h-[205px]">
          <Card :type="dependenciesThreats.type.toLowerCase()" :title="dependenciesThreats.findings[0].title"
            :rank="dependenciesThreats.findings[0].count" :growth="dependenciesThreats.findings[0].growth" />
        </div>
      </div>
    </div>
  </div>
