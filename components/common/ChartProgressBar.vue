<template>
  <div class="border border-1 border-lightGainsBor rounded-lg mt-3 sm:mt-6 xl:mt-0">
    <div class="flex justify-center flex-wrap relative pt-3 sm:pt-0">
      <div v-for="(value, index) in chartProgressBar" :key="index"
        class="w-full sm:w-4/12 p-4 md:p-6 text-center flex flex-col">
        ...
        <div>
          ...
          <div class="bg-lightGainsBor max-w-[272px] w-full h-1 rounded-2xl mx-auto"
            :class="getProgressBarClass(value.title)">
            <UProgress :value="value.rankPercentage" size="sm" :color="getProgressBarColor(value.title)" />
          </div>
        </div>
      </div>
      <span v-for="position in ['left', 'right']" :key="position"
        class="h-[72px] w-[1px] md:block bg-lightGainsBor absolute top-[30px] hidden"
        :class="[position === 'left' ? 'left-[33.5%]' : 'right-[33.5%]']"></span>
    </div>
  </div>
</template>
